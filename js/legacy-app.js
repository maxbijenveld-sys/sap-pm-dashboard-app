...
(function(){
  ...
  function place(el){
    // body heeft op desktop zoom:.8 (css/app.css) — getBoundingClientRect
    // geeft visuele (gezoomde) coördinaten terug, terwijl left/top op de
    // tooltip (zelf ook een gezoomd element) nogmaals met de factor
    // vermenigvuldigd worden. Alles omrekenen naar layout-pixels, anders
    // staat de tip merkbaar scheef. Leest de factor dynamisch uit (zie
    // regel hieronder), dus dit blijft kloppen als de zoomwaarde ooit weer
    // wijzigt.
...
// Zonder dit triggert elke toetsaanslag in een zoekveld een volledige
// her-render (KPI's herberekenen + tot 3 Chart.js-instanties slopen en
// opnieuw tekenen) — op de oudere fabriekstablets waar dit dashboard op
// draait (zie comment bij sortTh hieronder) merkbaar traag bij snel typen.
function debounce(fn,ms){
  let t=null;
  return (...args)=>{ clearTimeout(t); t=setTimeout(()=>fn(...args),ms); };
}
const renderDashDebounced=debounce(()=>renderDash(),200);
const renderKpiDetailDebounced=debounce(()=>renderKpiDetail(),200);
const renderBacklogDebounced=debounce(()=>renderBacklog(),200);
...
// Pagina's die onder de dashboard-permissie vallen (rechtenpoort in showPage
// hieronder) — dit is dus breder dan alleen de Onderhoud-tegel: PX Trend en
// SAP PM vallen er ook onder, want die vereisen dezelfde dashboard-toegang.
const DASHBOARD_PAGES=['dashboard','backlog','analyse','addweek','kpi-detail','week','rapport','betrouwbaarheid','pxtrend','pxrapport'];
// Pagina's die inhoudelijk bij de Onderhoud-tegel horen — smaller dan
// DASHBOARD_PAGES hierboven. Bepaalt alleen of de contextuele
// Onderhoud-subnav (Dashboard/Backlog/Analyse/+Week toevoegen) zichtbaar is;
// die hoort niet te verschijnen op andere tegels (PX Trend, SAP PM) die
// toevallig dezelfde dashboard-permissie delen.
const ONDERHOUD_SUBNAV_PAGES=['dashboard','backlog','analyse','addweek','kpi-detail','week','rapport'];
...
async function sbFetch(path,opts={}){
  ...
  if(!r.ok){
    ...
    // Backstop: als ensureFreshSession() de expiratie miste (bv. een
    // ingetrokken token, of een race met een lopend verzoek), dwing hier
    // alsnog een schone her-login af i.p.v. de rauwe "JWT expired" te tonen.
    if(r.status===401&&/jwt|token/i.test(msg)){
      forceReloginExpired();
...
async function loadWeeksFromDB(){
  // Bewust GEEN eigen try/catch die hier op [] terugvalt: de enige
  // aanroeper (initApp) kan een lege array dan niet onderscheiden van "het
  // laden is mislukt" en start in dat geval de seed-routine — die schrijft
  // hardcoded voorbeelddata naar de database. Een tijdelijke netwerkfout zou
  // zo stilzwijgend echte weekplanning kunnen overschrijven met demodata.
  // Laat de fout dus gewoon doorgooien; initApp's eigen catch vangt 'm op
  // zonder iets weg te schrijven.
  const data = await sbFetch('/rest/v1/weeks?select=*&order=year.asc,week.asc');
  weeks = (data || []).map(w => ({
    id: w.id,
    week: w.week,
    year: w.year,
...
async function saveWeekToDB(w){
  ...
  await sbFetch('/rest/v1/weeks',{
    ...
    body:JSON.stringify({id:w.id,week:w.week,year:w.year,label:w.label,rows:w.rows})
  });
}
...
// Dwingt een schone her-login af zodra de sessie écht niet meer te
// verversen is (refresh_token zelf verlopen/ingetrokken) — i.p.v. stilzwijgend
// door te gaan met een dode access_token en pas bij de eerstvolgende actie
// een rauwe "JWT expired" te tonen (en eventueel handmatig ingevoerde data
// kwijt te raken). window.__platformAuth is dezelfde objectreferentie die de
// financiënmodule als ctx vasthoudt, dus deze fix werkt daar automatisch mee.
function forceReloginExpired(){
  currentUser=null;isAdmin=false;weeks=[];
  window.__platformAuth=null;
  if(window.Platform&&window.Platform.onSignOut) window.Platform.onSignOut();
  localStorage.removeItem('sap_pm_session');
  showLoginScreen('Je sessie is verlopen — log opnieuw in.');
}
  ...
  _sessionRefreshing=(async()=>{
    try{
      ...
      if(r.ok&&d.access_token){
        ...
        // refresh_token zelf is verlopen/ongeldig — nog een keer proberen
        // heeft geen zin.
        forceReloginExpired();
      }
      // Overige statuscodes (bv. 500, netwerkbibliotheek-eigenaardigheden):
      // stil laten — mogelijk tijdelijk, de volgende interval/focus-poging
...
async function openMfaEnroll(required){
  // Guard tegen dubbelklikken: zonder deze vlag vuurt een tweede klik vóórdat
  // de eerste POST /auth/v1/factors is afgerond nog een parallelle aanroep
  // af, die een tweede (onbevestigde) TOTP-factor aanmaakt — de getoonde QR
  // kan dan bij een ander factorId horen dan waarmee later geverifieerd
  // wordt. Zelfde patroon als submitMfaEnroll/submitFinanceMfa, die de knop
  // wél al synchroon disablen vóór hun eerste await.
  if(_mfaEnrollOpening) return;
  _mfaEnrollOpening=true;
  try{
    await openMfaEnrollInner(required);
  } finally {
    _mfaEnrollOpening=false;
  }
}
async function openMfaEnrollInner(required){
...
function copyTempPassword(){
  const s=(document.getElementById('newUserTempPw')?.textContent||'').trim();
  if(!s||!navigator.clipboard) return;
  navigator.clipboard.writeText(s).then(()=>showGlobalStatus('Wachtwoord gekopieerd','success')).catch(()=>{});
}
...
async function handleLogin(){
  ...
  // De Enter-toets zit op de invoervelden (niet op de knop), dus btn.disabled
  // voorkomt geen dubbele aanroep via twee snelle Enter-drukken — alleen een
  // tweede klik. Zonder deze expliciete guard konden twee parallelle
  // signIn()+initApp()-aanroepen starten (en bij een lege database beide de
  // seed-routine draaien).
...
async function initApp(){
  ...
  try{
    ...
    // Bewust GEEN getDefaultWeeks() hier: dat verving eerder een mislukte
    // laadactie stilzwijgend door verouderde demodata (W19-W24), waardoor
    // een DB/schema-fout eruitzag als "de planning is leeg vanaf week 25"
    // in plaats van een zichtbare storing. Laat 'weeks' leeg en toon de
    // fout persistent (zie showGlobalStatus), zodat een echt laadprobleem
    // nooit verward kan worden met echte, actuele planningsdata.
    showGlobalStatus('Fout bij laden van weekdata: '+e.message+' — herlaad de pagina of neem contact op met de beheerder. Er wordt GEEN demodata getoond.','error');
    weeks=[];
  }
...
// Fouten ('error') verdwijnen bewust NIET vanzelf: een 7-seconden-flash liet
// eerder een mislukte weekdata-load (kolom 'weeks.year' bestond nog niet)
// onopgemerkt overgaan in stilzwijgend getoonde oude demodata — niemand zag
// het verschil tussen "dit is de echte planning" en "dit is een noodgreep".
let _statusTimer=null;
function showGlobalStatus(msg,type){
  const el=document.getElementById('globalStatus');
  if(!el)return;
  // Sluitknop: foutmeldingen verdwijnen bewust niet automatisch (zie
  // hieronder), maar moeten wel handmatig weg te klikken zijn — anders
  // blijft een allang opgeloste fout onbeperkt staan.
  el.innerHTML=`<div class="gstatus ${type||'info'}"><span>${msg}</span><button type="button" class="gstatus-close" onclick="this.parentElement.remove()" aria-label="Melding sluiten">&times;</button></div>`;
...
let _excelUploadGen=0;
async function handleExcelUpload(inp){
  if(!inp.files.length) return;
  if(!isAdmin){showGlobalStatus('Geen rechten om Excel te uploaden.','error');return;}
  const file=inp.files[0];
  // Generatie-token: als de gebruiker een tweede bestand kiest vóór het
  // inlezen van het eerste klaar is, mag alleen het LAATST gekozen bestand
  // uiteindelijk de weekkiezer/preview bepalen — zelfde patroon als
  // financeUpload.js (financiënmodule).
  const myGen=++_excelUploadGen;
  ...
  reader.onload=async e=>{
    try{
      ...
      if(myGen!==_excelUploadGen) return; // een later gekozen bestand is inmiddels leidend
      ...
      // Eén tabblad = een expliciete keuze van de gebruiker (bv. bewust één
      // week los geëxporteerd) — die gaat altijd door na een korte controle,
      // ongeacht welke week het is.
      if(alleWeken.length===1){
        const r=alleWeken[0];
        const gepland=r.rows.filter(x=>x.type==='gepland').length;
        const ongepland=r.rows.filter(x=>x.type==='ongepland').length;
        const wid='w'+r.yearNum+'-'+r.weekNum;
        const mark=weeks.findIndex(w=>w.id===wid)>=0?' (overschrijft bestaande week)':'';
        if(!confirm(`Controleer voor het opslaan:\n\nWeek ${r.weekNum}, ${r.yearNum}: ${r.rows.length} WO's — ${gepland} gepland, ${ongepland} ongepland${mark}\n\nDoorgaan?`)) return;
        await saveWeeksToDBAndRefresh(alleWeken);
        return;
      }

      // De master-Excel bevat doorgaans alle weken van het jaar. Vroeger werd
      // hieruit stilzwijgend alleen de "zojuist afgesloten" week verwerkt —
      // veilig voor de routine wekelijkse upload, maar onbruikbaar zodra
      // iemand bewust meerdere (historische) weken tegelijk wil corrigeren
      // én een nieuwe week wil toevoegen. Nu: laat expliciet zien wat er per
      // week verandert en laat de gebruiker kiezen welke week(en) verwerkt
      // worden, met de zojuist afgesloten + alles wat nieuw/gewijzigd is
      // vast aangevinkt (zodat de routine-upload nog steeds in één klik kan).
      renderWeekPicker(alleWeken);
    } catch(err){
...
// Rij-sleutel voor de per-week diff: dag + ordernummer + omschrijving.
function weekRowKey(r){return[r.day,r.wo,r.omschrijving].join('|');}
function diffWeekRows(oldRows,newRows){
  const oldMap=new Map((oldRows||[]).map(r=>[weekRowKey(r),r]));
  const newMap=new Map((newRows||[]).map(r=>[weekRowKey(r),r]));
...
let _pendingWeekUpload=null;
function renderWeekPicker(alleWeken){
  _pendingWeekUpload=alleWeken;
  const doel=isoWeekJustFinished(new Date());
  const rows=alleWeken.map(r=>{
    const wid='w'+r.yearNum+'-'+r.weekNum;
    const prev=weeks.find(w=>w.id===wid);
    let statusTxt,statusCls,isSame=false;
    if(!prev){statusTxt='nieuw';statusCls='#56d364';}
    else if(prev.label===r.label&&JSON.stringify(prev.rows)===JSON.stringify(r.rows)){
      statusTxt='gelijk';statusCls='#8b949e';isSame=true;
    }else{
      const d=diffWeekRows(prev.rows,r.rows);
      statusTxt=`gewijzigd — +${d.added.length} −${d.removed.length} ~${d.changed.length}`;statusCls='#d29922';
    }
    const isDoel=r.weekNum===doel.week&&r.yearNum===doel.year;
    const checked=(isDoel||!isSame)?' checked':'';
    // data-week draagt jaar+week samen (niet alleen weeknummer) — anders zou
    // confirmWeekPicker() twee verschillende jaargangen met hetzelfde
    // weeknummer niet kunnen onderscheiden.
    return `<label style="display:flex;align-items:center;gap:8px;padding:6px 4px;border-bottom:1px solid var(--color-border)">
      <input type="checkbox" class="wp-chk" data-week="${r.yearNum}-${r.weekNum}"${checked}>
      <span style="flex:1">Week ${r.weekNum}, ${r.yearNum} — ${r.rows.length} WO's${isDoel?' <span style="color:#8b949e">(zojuist afgesloten)</span>':''}</span>
      <span style="font-size:11px;color:${statusCls}">${statusTxt}</span>
    </label>`;
  }).join('');

  document.getElementById('uploadStatus').innerHTML=`
    <div class="note" style="margin-bottom:12px">
      <div style="font-weight:600;margin-bottom:8px">Dit bestand bevat ${alleWeken.length} weken. Vink aan welke je wilt opslaan — standaard staan de zojuist afgesloten week en alles wat nieuw of gewijzigd is al aan.</div>
      <div style="max-height:280px;overflow-y:auto;margin-bottom:8px">${rows}</div>
      <div style="display:flex;gap:8px;align-items:center">
        <button type="button" class="btn" onclick="document.querySelectorAll('.wp-chk').forEach(c=>c.checked=true)">Alles</button>
        <button type="button" class="btn" onclick="document.querySelectorAll('.wp-chk').forEach(c=>c.checked=false)">Niks</button>
        <span style="flex:1"></span>
        <button type="button" class="btn" onclick="cancelWeekPicker()">Annuleren</button>
        <button type="button" class="btn btn-p" onclick="confirmWeekPicker()">Opslaan</button>
      </div>
    </div>`;
}
function cancelWeekPicker(){
  _pendingWeekUpload=null;
  document.getElementById('uploadStatus').innerHTML='';
}
async function confirmWeekPicker(){
  const gekozen=[...document.querySelectorAll('.wp-chk:checked')].map(c=>c.dataset.week);
  const results=(_pendingWeekUpload||[]).filter(r=>gekozen.includes(r.yearNum+'-'+r.weekNum));
  _pendingWeekUpload=null;
  if(!results.length){document.getElementById('uploadStatus').innerHTML='';return;}
  await saveWeeksToDBAndRefresh(results);
}

// Gedeeld door de single-sheet-flow en de weekkiezer: slaat de gekozen weken
// op, ververst de UI en toont een resultaatmelding.
async function saveWeeksToDBAndRefresh(results){
  // Zelfde principe als saveWeek()/delWeek(): de lokale state pas muteren ná
  // een bevestigde opslag, nooit ervoor. Eerder gooide een netwerkfout
  // halverwege een multi-week-import onafgehandeld (geen try/catch bij de
  // aanroepers), terwijl de lokale `weeks`-array al wél de niet-opgeslagen
  // week bevatte — dus de UI toonde data die niet echt in de database stond.
  // saveWeekToDB is een idempotente upsert per week, dus één mislukte week
  // hoeft de rest van de batch niet te blokkeren.
  let added=0,updated=0;
  const mislukt=[];
  for(const result of results){
    const wid='w'+result.yearNum+'-'+result.weekNum;
    const newWeek={id:wid,week:result.weekNum,year:result.yearNum,label:result.label,rows:result.rows};
    try{
      await saveWeekToDB(newWeek);
    }catch(e){
      mislukt.push(`${result.weekNum} (${result.yearNum})`);
      continue;
    }
  ...
  window._installatieStats=null; // cache is nu verouderd — zie probleemInstallatieSet()
  rebuildWkPages();refreshFilter();refreshTabs();renderDash();

  if(mislukt.length){
    showUploadStatus(`${added+updated} week(en) opgeslagen, maar week ${mislukt.join(', ')} kon niet worden opgeslagen door een netwerk-/serverfout — probeer die week opnieuw te uploaden.`,'error');
    return;
  }
  if(results.length===1){
    showUploadStatus(`Week ${results[0].weekNum} geladen en opgeslagen — ${results[0].rows.length} werkorders.`,'success');
    showWkPg('w'+results[0].yearNum+'-'+results[0].weekNum);
  } else {
...
// Fouten ('error') verdwijnen bewust NIET vanzelf — zelfde reden als bij
// showGlobalStatus hierboven: een upload die mislukte (bv. "week kon niet
// worden opgeslagen door een netwerk-/serverfout") mag niet stil verdwijnen
// voordat iemand het gezien heeft.
...
// SAP-ordernummerconventie op deze site: een ordernummer dat begint met een
// 3 is een PM03-order (inspectieronde), geen herstel/onderhoud. Een hoog
// "% afgerond" kan zo grotendeels uit afgevinkte inspecties bestaan i.p.v.
// daadwerkelijk verholpen storingen — kpis() splitst dat daarom apart uit
// (zie het weekrapport, js/report.js) i.p.v. alle "ja"-statussen op één hoop
// te gooien.
function isInspectieOrder(wo){ return /^3/.test((wo||'').toString().trim()); }
...
function kpis(rows){
  const t=rows.length,ja=rows.filter(r=>r.status==='ja').length,
    ...
    jaInspectie=rows.filter(r=>r.status==='ja'&&isInspectieOrder(r.wo)).length,
    jaHerstel=ja-jaInspectie,
    pctInspectieVanAfgerond=ja>0?Math.round(jaInspectie/ja*100):0;
  return{t,ja,nee,d,g,o,pct,jaInspectie,jaHerstel,pctInspectieVanAfgerond};
}
...
// Za/Zo staan hier ook in (naast de reguliere Ma-Vr) voor het zeldzame geval
// van een extra weekenddagblok in de bron-Excel (parser.js labelt zo'n blok
// met de echte kalenderdag i.p.v. het te laten samenvallen met Vr) — anders
// sorteert zo'n regel altijd naar voren (DAY_ORDER[...]||0).
const DAY_ORDER={Ma:1,Di:2,Wo:3,Do:4,Vr:5,Za:6,Zo:7,Vulwerk:8};
const BASISDAGEN=['Ma','Di','Wo','Do','Vr'];
// Basisdagen + een eventuele extra dag die daadwerkelijk in de regels
// voorkomt (bv. een zaterdagploeg) — zo blijft een normale week gewoon de
// 5 vertrouwde dag-tabs/kolommen tonen, maar verdwijnt een uitzonderlijke
// weekenddag niet stilzwijgend uit de per-dag-weergaven.
function dagenVoor(rows){
  const extra=[...new Set((rows||[]).map(r=>r.day))].filter(d=>d&&!BASISDAGEN.includes(d));
  extra.sort((a,b)=>(DAY_ORDER[a]||99)-(DAY_ORDER[b]||99));
  return BASISDAGEN.concat(extra);
}
function groupRowsByOrder(){
  const byOrder={},loose=[];
  weeks.forEach(w=>{
    w.rows.forEach(r=>{
      const row={...r,_week:w.week,_year:w.year,_wk:`W${w.week}`};
      if(!r.wo||r.wo==='—'){loose.push(row);return;}
      (byOrder[r.wo]=byOrder[r.wo]||[]).push(row);
    });
  });
  Object.values(byOrder).forEach(e=>e.sort((a,b)=>(weekIndex(a._year,a._week)-weekIndex(b._year,b._week))||(DAY_ORDER[a.day]-DAY_ORDER[b.day])));
...
// security: ' toegevoegd naast &<>" — esc()-output wordt op meerdere plekken
// geïnterpoleerd in onclick="...('${esc(x)}')" met enkele quotes als JS-
// stringdelimiter (bv. verwijderGebruiker, showDayTbl). Zonder deze escape
// breekt een waarde met een apostrof (bv. een naam als "O'Brien") uit de
// string en kan in theorie JS injecteren.
...
function computeKpiRows(wf,tf){
  let fWks=wf==='all'?weeks:weeks.filter(w=>w.id===wf);
  if(wf==='all'){
    // Toekomstige weken die alleen alvast ingeplande (nog niet uitgevoerde)
    // regels bevatten laten de "Alle weken"-grafieken doortrekken tot ver
    // voorbij de laatste week waar echt iets is afgerond — verwarrend, geen
    // extra informatie. Knip daarom af ná de laatste week met minstens 1
    // afgeronde ("ja") WO, ongeacht het huidige type-filter (zodat wisselen
    // tussen Gepland/Ongepland de zichtbare weekrange niet laat springen).
    let laatsteMetDataIdx=-1;
    fWks.forEach((w,i)=>{ if(filtered(w,'all').some(r=>r.status==='ja')) laatsteMetDataIdx=i; });
    if(laatsteMetDataIdx>=0) fWks=fWks.slice(0,laatsteMetDataIdx+1);
  }
  let allRows=[];
  fWks.forEach(w=>filtered(w,tf).forEach(r=>allRows.push({...r,_wk:`W${w.week}`,_week:w.week,_year:w.year})));
  let kpiRows=allRows;
  if(wf==='all'){
...
function renderDash(){
  ...
  const sortedWeeks=[...weeks].sort((a,b)=>(a.year-b.year)||(a.week-b.week));
  ...
  if(singleWeek){
    ...
    pctD=DAYS.map(d=>kpis(filtered(w,tf).filter(x=>x.day===d)).pct);
    let running=0;
    ...
    pctD=fWks.map(w=>kpis(filtered(w,tf)).pct);
    let running=0;
    cumD=jaD.map(v=>{running+=v;return running;});
  ...
  tableRows=applySort(tableRows,_dashSort,{
    week:r=>weekIndex(r._year,r._week), day:r=>DAY_ORDER[r.day]||0, order:r=>r.wo||'', omschrijving:r=>r.omschrijving||'',
...
function enrichWithHistory(rows){
  ...
  return rows.map(r=>{
    ...
    // weekIndex (jaar+week samen, zie parser.js) i.p.v. kaal weeknummer-
    // verschil: dat laatste werd rond een jaarwisseling negatief of zwaar
    // onderschat (bv. wk51->wk2 gaf 2-51+1=-48). Math.max(1,...) blijft als
    // laatste vangnet voor extreme/onverwachte data.
    const weeksOpenLatest=last.status!=='ja'?Math.max(1,weekIndex(last._year,last._week)-weekIndex(first._year,first._week)+1):0;
...
function renderKpiDetail(){
  ...
  shown.forEach(r=>{
    ...
    th+=`<tr>
      ...
      <td data-label="Actie"><button class="btn" style="padding:4px 10px;font-size:12px" onclick="showWkPg('w${r._year}-${r._week}',true)">Ga naar W${r._week} &rsaquo;</button></td>
    </tr>`;
  });
...
function renderBacklog(){
  ...
  Object.entries(byOrder).forEach(([wo,entries])=>{
    ...
    // Zelfde weekIndex-aanpak als enrichWithHistory hierboven.
    const weeksOpenLatest=last.status!=='ja'?Math.max(1,weekIndex(last._year,last._week)-weekIndex(first._year,first._week)+1):0;
...
function syncOnderhoudSubnav(id){
  ...
  if(!nav) return;
  nav.style.display=ONDERHOUD_SUBNAV_PAGES.includes(id)?'flex':'none';
...
// P0-audit: het handmatige formulier valideerde een ingevoerd ordernummer
// op geen enkele manier — dit heeft al eerder tot een echt data-incident
// geleid (zie findOrderReferenceMismatch/orderEntrySignature in risk.js,
// gebouwd om zo'n afwijking áchteraf in de Backlog te signaleren). Dit
// waarschuwt er nu vóór het opslaan al voor, in het formulier zelf.
function checkOrderMismatch(rowDiv){
  const warnEl=rowDiv.querySelector('.rw-warn');
  if(!warnEl) return;
  const wo=(rowDiv.querySelector('.rw').value||'').trim();
  const oms=(rowDiv.querySelector('.ro').value||'').trim();
  if(!wo||wo==='—'||!oms||typeof groupRowsByOrder!=='function'||typeof orderEntrySignature!=='function'){
    warnEl.style.display='none';return;
  }
  const {byOrder}=groupRowsByOrder();
  const bestaand=byOrder[wo];
  if(!bestaand||!bestaand.length){warnEl.style.display='none';return;}
  const nieuweSig=orderEntrySignature({omschrijving:oms});
  const afwijkend=bestaand.find(e=>orderEntrySignature(e)!==nieuweSig);
  if(afwijkend){
    warnEl.style.display='block';
    warnEl.innerHTML=`&#9888; Ordernummer ${esc(wo)} komt al voor (W${afwijkend._week}) met omschrijving &ldquo;${esc(afwijkend.omschrijving)}&rdquo; — is dit dezelfde klus?`;
  } else {
...
function addRow(){
  ...
  d.innerHTML=`<select class="rd" style="${si}"><option>Ma</option><option>Di</option><option>Wo</option><option>Do</option><option>Vr</option><option>Za</option><option>Zo</option></select>
    ...
    <div class="rw-warn" style="display:none;grid-column:1/-1;font-size:11px;color:#d29922;padding:2px 2px 4px"></div>`;
  c.appendChild(d);
  const check=()=>checkOrderMismatch(d);
  d.querySelector('.rw').addEventListener('blur',check);
  d.querySelector('.ro').addEventListener('blur',check);
}

async function saveWeek(){
  if(!isAdmin){showGlobalStatus('Geen rechten om weken toe te voegen.','error');return;}
  const yr=parseInt(document.getElementById('nYear').value)||new Date().getFullYear();
  ...
  const wid='w'+yr+'-'+wn;
  if(weeks.find(w=>w.id===wid)){alert(`Week ${wn} (${yr}) bestaat al`);return;}
  ...
  const newWeek={id:wid,week:wn,year:yr,label:lbl,rows};
  try{
    await saveWeekToDB(newWeek);
    weeks.push(newWeek);
    weeks.sort((a,b)=>(a.year-b.year)||(a.week-b.week));
...
window.addEventListener('DOMContentLoaded',async()=>{
  // Jaar-veld van "Week toevoegen" op het huidige jaar i.p.v. een
  // hardcoded jaartal — anders krijgt elke handmatig ingevoerde week na de
  // jaarwisseling zonder waarschuwing het verkeerde jaartal.
  const nYearEl=document.getElementById('nYear');
  if(nYearEl) nYearEl.value=new Date().getFullYear();
  ...
  try{
    ...
    authUsers.forEach(u=>{
      ...
      html+=`<tr>
        ...
        <td data-label="Actie">${isZelf?'<span style="font-size:11px;color:#aaa">(jijzelf)</span>':`<div style="display:flex;gap:6px;flex-wrap:wrap"><button class="btn" style="font-size:11px;padding:3px 8px" onclick="resetMfaGebruiker('${esc(u.id)}','${esc(u.email)}')" title="Verwijdert de gekoppelde authenticator-app, zodat de gebruiker opnieuw kan enrollen">2FA resetten</button><button class="btn btn-del" style="font-size:11px;padding:3px 8px" onclick="verwijderGebruiker('${esc(u.id)}','${esc(u.email)}')">Verwijderen</button></div>`}</td>
      </tr>`;
    });
  ...
  try{
    ...
    status.innerHTML=`<div style="background:#12261a;border:1px solid #2ea043;border-radius:8px;padding:12px 16px">
      ...
      <div style="background:#21262d;border-radius:6px;padding:10px 14px;font-family:monospace;font-size:13px;color:#e6edf3">
        ...
        <div style="display:flex;align-items:center;gap:8px">Wachtwoord: <strong id="newUserTempPw">${tempPw}</strong><button type="button" class="logout-btn" style="padding:2px 8px" onclick="copyTempPassword()" title="Kopieer wachtwoord">&#128203;</button></div>
...
// P0-audit: er bestond geen herstelpad als een gebruiker zijn 2FA-apparaat
// kwijtraakt/vervangt — Financiën bleef dan blijvend ontoegankelijk zonder
// een rechtstreekse database-ingreep. Verwijdert de gekoppelde factor(en)
// server-side (api/admin.js, actie reset-mfa), waarna de gebruiker bij de
// volgende Financiën-ontgrendeling gewoon opnieuw kan enrollen.
async function resetMfaGebruiker(userId,email){
  if(!isAdmin){showGlobalStatus('Geen rechten om 2FA te resetten.','error');return;}
  if(!confirm(`2FA resetten voor ${email}?\n\nDe gekoppelde authenticator-app wordt losgekoppeld — de gebruiker moet bij de volgende Financiën-ontgrendeling opnieuw een nieuwe QR-code scannen.`)) return;
  try{
    const r=await fetch(`/api/admin?action=reset-mfa&userId=${encodeURIComponent(userId)}&email=${encodeURIComponent(email)}`,{
      method:'POST',
      headers:{'Authorization':'Bearer '+currentUser?.access_token}
    });
    const data=await r.json().catch(()=>({}));
    if(!r.ok) throw new Error(data.error||`Fout ${r.status}`);
    showGlobalStatus(data.factorsRemoved>0?`2FA gereset voor ${email} (${data.factorsRemoved} koppeling${data.factorsRemoved===1?'':'en'} verwijderd).`:`Geen actieve 2FA gevonden voor ${email}.`,'success');
  }catch(e){showGlobalStatus('2FA resetten mislukt: '+e.message,'error');}
}
...
function renderInzichten(){
  ...
  if(weeks.length>=3){
    ...
    const pcts=last3.map(w=>kpis(w.rows).pct);
...
function computeRedenData(rows){
  const redenCounts={};
  const redenRowsMap={};
  const source=rows||weeks.flatMap(w=>(w.rows||[]).map(row=>({...row,_wk:`W${w.week}`,_week:w.week,_year:w.year})));
  source.forEach(row=>{
...
function findStoringMatches(row){
  const w=weeks.find(w=>w.week===row._week&&w.year===row._year);
