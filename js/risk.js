...
// Zelfde weekIndex als parser.js — hier gedupliceerd (zelfde reden als
// riskInstallatieCode hieronder: in de browser zijn parser.js/risk.js
// classic scripts die een globale scope delen, maar in Node (deze test-
// suite) heeft elk bestand zijn eigen module-scope, dus een kale aanroep
// zonder duplicatie/require zou hier ReferenceError geven). Bewust
// identiek gehouden; wijzigt de definitie in parser.js, dan hier ook.
...
function computeRisk(entries, probleemInstallaties) {
  ...
  // weekIndex (jaar+week samen, parser.js) i.p.v. kaal weeknummer-verschil —
  // dat laatste zou rond een jaarwisseling negatief of zwaar onderschat
  // worden (bv. wk51->wk2 gaf 2-51+1=-48), waardoor een al maanden
  // openstaande order plots als "net geopend" zou scoren.
  const wkOpen = Math.max(1, weekIndex(last._year, last._week) - weekIndex(first._year, first._week) + 1);
