...
(function(){
  ...
  function build(){
    let wks=(typeof weeks!=='undefined'?weeks:[]).slice().sort((a,b)=>(a.year-b.year)||(a.week-b.week));
    if(!wks.length) return '<div class="wf">Nog geen weekdata geladen.</div>';
    // Toekomstige/placeholder-weken zonder enige afgeronde WO (bv. alvast
    // aangemaakte planningsweken) horen niet mee te tellen als "recent" —
    // kap af bij de laatste week met minstens 1 afgeronde WO.
    let laatsteMetDataIdx=-1;
    wks.forEach((w,i)=>{ if((w.rows||[]).some(r=>r.status==='ja')) laatsteMetDataIdx=i; });
    if(laatsteMetDataIdx>=0) wks=wks.slice(0,laatsteMetDataIdx+1);
    ...
    // Info-icoontje met uitleg-tooltip (native title-attribuut, zelfde patroon
    // als de risicobadges elders in de app) — puur ter verduidelijking van
    // hoe een KPI is berekend, met name waar dat niet vanzelf spreekt (bv.
    // "Waarvan inspectie" telt alleen mee binnen de al afgeronde WO's).
    const infoIcon=info=>info?`<span class="rpt-info" title="${esc(info)}" aria-label="${esc(info)}"><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><circle cx="12" cy="12" r="10"/><line x1="12" y1="16" x2="12" y2="12"/><line x1="12" y1="8" x2="12.01" y2="8"/></svg></span>`:'';
    const kpiCell=(l,v,s,info)=>`<div class="rpt-kpi"><div class="rpt-kpi-l">${l}${infoIcon(info)}</div><div class="rpt-kpi-v">${v}</div><div class="rpt-kpi-s">${s||''}</div></div>`;
    ...
    <div id="rptDoc">
      ...
      <div class="wf rpt-sec">
        ...
        ${k.ja>0?` Daarvan was <strong>${k.jaInspectie}</strong> reguliere inspectie (PM03, ordernummer begint met 3) en <strong>${k.jaHerstel}</strong> daadwerkelijk herstel/onderhoud.`:''}
        ...
        <div class="rpt-kpis">
          ${kpiCell('Werkorders',k.t,`${k.g} gepland · ${k.o} ongepland`,'Alle werkorders van deze week: geplande werkzaamheden plus werk dat ongepland is bijgekomen (storingen e.d.).')}
          ${kpiCell('Afgerond',`${k.pct}%`,`${k.ja} van ${k.t}`,'Percentage werkorders met status "Ja" (afgerond), berekend over álle werkorders van deze week — gepland én ongepland samen.')}
          ${kpiCell('Waarvan inspectie',`${k.pctInspectieVanAfgerond}%`,`${k.jaInspectie} PM03 · ${k.jaHerstel} herstel`,'Van de afgeronde werkorders hierboven: welk deel is een PM03-order (ordernummer begint met een 3) — preventieve orders, geen storingsherstel. PM03-orders die nog niet zijn afgerond tellen hier niet in mee.')}
          ${kpiCell('Niet uitgevoerd',k.nee,'status Nee','Werkorders met status "Nee": deze week niet uitgevoerd, meestal met een reden (zie de tabel hieronder).')}
          ${kpiCell('Deels / UC',k.d,'gestart, niet af','Werkorders die wél gestart zijn maar nog niet klaar: status "Deels" of "u.c." (under construction).')}
          ${kpiCell('Backlog open',openTotaal,'unieke WO’s','Aantal unieke werkorders dat over de héle planning (alle weken samen) nog openstaat — niet beperkt tot deze week.')}
        </div>
      </div>

      <div class="wf rpt-sec">
        <div class="sl" style="margin-top:0">Trend — laatste ${trend.length} weken</div>
        <div class="tw keep-table"><table><thead><tr><th>Week</th><th style="text-align:right">WO's</th><th style="text-align:right">Afgerond</th><th style="text-align:right">Nee</th><th style="text-align:right">Deels/UC</th><th style="text-align:right">Ongepland</th><th style="text-align:right">% afgerond</th><th style="text-align:right">Waarvan insp.</th></tr></thead><tbody>
          ${trend.map(t=>`<tr${t.week===cur.week?' style="font-weight:600"':''}><td>W${t.week}</td><td style="text-align:right">${t.t}</td><td style="text-align:right">${t.ja}</td><td style="text-align:right">${t.nee}</td><td style="text-align:right">${t.d}</td><td style="text-align:right">${t.o}</td><td style="text-align:right">${t.pct}%</td><td style="text-align:right">${t.ja>0?t.pctInspectieVanAfgerond+'%':'—'}</td></tr>`).join('')}
        </tbody></table></div>
      </div>
