...
(function(){
  ...
  function build(){
    ...
    const wks=(magDash&&typeof weeks!=='undefined'?weeks:[]).slice().sort((a,b)=>(a.year-b.year)||(a.week-b.week));
    ...
    if(cur){
      ...
      const pctSpark=wks.slice(-6).map(w=>kpis(w.rows).pct);
      ...
      Object.values(byOrder).forEach(entries=>{
        ...
        // Jaar-bewust (weekIndex uit risk.js): een kaal weeknummer-verschil
        // ondertelt bij een jaarwisseling (bv. W51 2026 -> W2 2027 zou anders
        // -48 geven i.p.v. 5) — zelfde aanpak als backlog/risicoscore.
        if((weekIndex(last._year,last._week)-weekIndex(first._year,first._week)+1)>=3) lang++;
      });

      signalHtml.uitvoering=sig('var(--color-primary)',`Uitvoering W${cur.week}`,k.pct+'%',deltaTxt,pctSpark,'#3fb950',"showPage('dashboard')");
      // Accent alleen alarmerend als de waarde daadwerkelijk aandacht
      // vraagt — anders (0/gunstig) een neutrale streep, zelfde principe
      // als mk() al toepast op de modulekaarten hieronder. Een schone
      // backlog hoort niet nog steeds rood te ogen op Start.
      signalHtml.risico=sig(hoog>0?'var(--color-danger-strong)':'var(--color-border)','Backlog — hoog risico',hoog,`van ${open} open werkorders`,null,null,"openBacklogFiltered('risico')");
      signalHtml.lang=sig(lang>0?'var(--color-warning)':'var(--color-border)','&#8805; 3 weken open',lang,'structurele aandacht nodig',null,null,"openBacklogFiltered('long')");
      ...
      signalHtml.ongepland=sig(k.o>0?'var(--color-warning)':'var(--color-border)','Ongepland / storingen',k.o,`deze week (W${cur.week})`,null,null,"showPage('dashboard')");
      signalHtml.terugkerend=sig(terugkerend>0?'var(--color-warning)':'var(--color-border)','Terugkerende WO&rsquo;s',terugkerend,'2+ weken achter elkaar',null,null,"openBacklogFiltered('all')");
      signalHtml.refmismatch=sig(refmismatch>0?'var(--color-danger-strong)':'var(--color-border)','Data-kwaliteit',refmismatch,refmismatch===1?'ordernummer met wisselende omschrijving':'ordernummers met wisselende omschrijving',null,null,"openBacklogFiltered('all')");
    }
    ...
    {
      const pxApi=window.__pxConditie;
      const pxClick="showPage('pxtrend');window.__initPxTrend&&window.__initPxTrend()";
      const pxRecords=pxApi&&typeof pxApi.buildInstallationHealth==='function'?pxApi.buildInstallationHealth():[];
      let pxBody;
      if(pxRecords.length){
        const counts={normaal:0,monitoren:0,onderzoeken:0,actie:0,onvoldoende:0};
        pxRecords.forEach(r=>counts[r.status]++);
        pxBody=`<div class="kg" style="grid-template-columns:repeat(auto-fit,minmax(90px,1fr));gap:8px">
          ${['normaal','monitoren','onderzoeken','actie','onvoldoende'].map(st=>`<div class="kc" style="--accent:${pxApi.STATUS_COLOR[st]};padding:10px 12px">
            <div class="kl" style="font-size:10px;white-space:nowrap">${pxApi.STATUS_EMOJI[st]} ${pxApi.STATUS_LABEL[st]}</div>
            <div class="kv" style="font-size:20px">${counts[st]}</div>
          </div>`).join('')}
        </div>
        <div class="hm-note" style="margin-top:8px">EL2310 (bakkenelevator): 9 jaar echte meetdata, 2 afwijkende stilstanden automatisch gevonden (najaar 2024) &mdash; de overige 269 installaties wachten nog op een gekoppelde meethistorie.</div>`;
      } else {
        pxBody=`<div class="hm-note">Nog geen conditiedata geladen.</div>`;
      }
      cards.push(modCard({accent:'var(--color-info)',icon:IC.pulse,titel:'PX Trend — conditie',
        desc:'Doorlopende proceswaarden per installatie: temperatuur, trilling, druk — met vroegsignalering',
        badge:'In ontwikkeling',badgeCls:'hm-b-dev',
        body:pxBody,cta:'Bekijk conditiebewaking &rsaquo;',meta:'EL2310/BU4930 echt &middot; rest structuur echt',
        clickJs:pxClick}));
    }
    ...
    cards.push(modCard({accent:'var(--color-warning)',icon:IC.db,titel:'SAP PM — werkorders &amp; meldingen',
      ...
      cta:'Bekijk voorbeeldweergave &rsaquo;',meta:'automatische aanlevering &middot; illustratieve cijfers',
      clickJs:"showPage('betrouwbaarheid');window.__initBetrouwbaarheid&&window.__initBetrouwbaarheid()"}));
