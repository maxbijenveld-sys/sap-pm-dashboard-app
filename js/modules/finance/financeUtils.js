...
// ISO-8601-weeknummer van een datum — gedupliceerd van parser.js (zelfde reden
// als weekIndex in js/risk.js: dit bestand draait zowel in de browser als in
// Node-scripts/tests en heeft dus geen gegarandeerde toegang tot parser.js'
// globale scope). Gebruikt om elke financiën-upload een weeknummer te geven:
// de MIP-export zelf is maand-gebaseerd ("Hier de maand invullen"), maar
// wordt in de praktijk wekelijks opnieuw geüpload — zonder een weeknummer
// zijn twee uploads binnen dezelfde maand niet van elkaar te onderscheiden
// in de versiegeschiedenis (identiek label, overlappende grafiekpunten).
...
// Jaar-bewuste, monotone week-index — gedupliceerd van js/risk.js (zelfde
// reden als isoWeekOf hierboven: geen gegarandeerde toegang tot risk.js'
// globale scope vanuit een ES-module). Gebruikt om "laatst bekende week"
// over een jaargrens heen correct te vergelijken (een kaal weeknummer-
// verschil zou bv. W2 2027 als ouder dan W51 2026 behandelen).
export function weekIndex(year, week) { return (Number(year) || 0) * 53 + (Number(week) || 0); }
...
// Reconciliatie-tolerantie: hoeveel twee onafhankelijk berekende totalen
// (bv. som van regels vs. een Excel-kopregel) uit elkaar mogen liggen voordat
// het als een echte afwijking geldt i.p.v. afrondingsruis. Voorheen 2×
// hardgecodeerd (financeParser.js en financeSources.js) met hetzelfde getal
// maar zonder gedeelde naam — nu één plek voor een bewuste aanpassing.
export const RECON_TOLERANCE_PCT = 0.005;
export function withinReconTolerance(diff, reference) {
  return Math.abs(diff) <= Math.max(1, Math.abs(reference) * RECON_TOLERANCE_PCT);
}

// "Actueel" (CCTR + KOB1 + personeel, dus alle geboekte kosten exclusief het
// nog-niet-gefactureerde obligo KOB2) — gebruikt door de Analyse-tabellen,
// de grafieken en de Excel-export. Voorheen 3× los gedupliceerd (dashboard,
// charts, export); nu één plek zodat een toekomstige aanpassing van "wat is
// actueel" niet op drie plekken los moet worden doorgevoerd.
export function actueelEntries(entries) {
  return (entries || []).filter(e => ACTUAL_BRON.includes(e.bron) || e.bron === 'personeel');
}

const eur = new Intl.NumberFormat('nl-NL', { style: 'currency', currency: 'EUR', maximumFractionDigits: 0 });
export function fmt(n) { return (n == null || isNaN(n)) ? '—' : eur.format(n); }
// ' meegenomen naast &<>" — consistent met de esc() in legacy-app.js/
// betrouwbaarheid.js: zonder deze escape kan een waarde met een apostrof
// uit een onclick="...('${esc(x)}')"-string breken. Niet bevestigd dat dit
// bestand vandaag zo'n patroon gebruikt, maar de helper wordt breed
// hergebruikt — inconsistent escapen tussen kopieën is een risico bij
// toekomstig hergebruik, niet iets om per losse aanroepplek te bewaken.
...
// Werkelijk geboekte kosten per SAP-order (CCTR/KOB1) — voor het koppelen
// van een RBM-project aan zijn échte realisatie. De RBM-tab wordt periodiek
// handmatig bijgewerkt en loopt daardoor soms achter op wat er al in SAP op
// diezelfde order geboekt staat (bv. een net afgeronde klus die in de
// RBM-tab nog op "0 realisatie" staat terwijl CCTR/KOB1 de echte kosten al
// heeft). KOB2 (obligo) telt hier bewust niet mee: dat is nog niet
// gefactureerd, dus geen realisatie.
export function actualByOrder(entries) {
  const m = {};
  for (const e of (entries || [])) {
    if (!e.order_nr || !ACTUAL_BRON.includes(e.bron)) continue;
    m[e.order_nr] = (m[e.order_nr] || 0) + (Number(e.bedrag_eur) || 0);
  }
  return m;
}
...
// Weeknummer-suffix voor een upload-label, afgeleid uit created_at (niet uit
// het opgeslagen label) — zo worden ook uploads van vóór de label-fix (die
// het weeknummer nog niet meekregen bij het opslaan) met terugwerkende
// kracht onderscheidbaar. Eén plek, gedeeld door de historiek-grafiek en de
// periodevergelijking-dropdown/delta-labels, zodat een upload overal
// hetzelfde ("... (wk N)") label draagt.
export function labelWithWeek(u) {
  const baseLabel = u.label || (u.periode ? `${MAAND[u.periode]} ${u.jaar}` : String(u.jaar));
  const week = u.created_at ? isoWeekOf(new Date(u.created_at)).week : null;
  return (week && !/\(wk\s*\d+\)/i.test(baseLabel)) ? `${baseLabel} (wk ${week})` : baseLabel;
}
...
export function versionHistory(currentUpload, others) {
  ...
  const rows = all.map(u => {
    ...
    // Weeknummer altijd afleiden uit created_at i.p.v. het opgeslagen label
    // — zonder dit clusterden meerdere uploads binnen dezelfde maand op
    // identieke labels/x-as-posities, wat de lijn er onnodig grillig liet
    // uitzien.
    const label = labelWithWeek(u);
    return {
