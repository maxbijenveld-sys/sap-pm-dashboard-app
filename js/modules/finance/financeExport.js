...
import { groupSum, computeKpis, actueelEntries } from './financeUtils.js';
...
export function buildSheets({ entries, budget, projects, orderMap }) {
  ...
  // Export is een (mogelijk audit-)document — geen top-N-afkap zoals in de
  // KPI-detailpanelen. Infinity i.p.v. een vast getal: bij >1000 distincte
  // werkorders viel het sheet voorheen stil terug tot de top-1000 duurste,
  // zonder dat te vermelden.
  const orders = [['Order', 'Omschrijving (onderhoud)', 'Status', 'Type', 'Kosten (EUR)'],
    ...costPerOrder(entries, orderMap, Infinity).map(o => [o.order, o.omschrijving || '', o.status || '', o.type || '', round(o.bedrag)])];
...
export function exportExcel(data, filename) {
  if (!window.XLSX) {
    // Zelfde statusbalk als de rest van de app (legacy-app.js) i.p.v. een
    // kale alert() die met dat patroon breekt.
    if (typeof window.showGlobalStatus === 'function') window.showGlobalStatus('Excel-bibliotheek niet geladen — herlaad de pagina en probeer opnieuw.', 'error');
    return;
  }
