...
import { renderDashboard, prefetchDashboard, invalidateDashboardCache, clearFinancePreferences, plantName } from './financeDashboard.js';
...
// P0-audit: er was geen manier om een per ongeluk bevestigde, verkeerde
// upload terug te draaien — die werd direct de voor iedereen getoonde
// "laatste versie" zonder correctiepad. Alleen de MEEST RECENTE upload PER
// PLANT mag verwijderd worden (niet zomaar oudere historie — die voedt nog
// steeds de periodevergelijking en het verloop-over-tijd), en alleen door
// een finance-admin.
function renderUploadsList(el, uploads) {
  ...
  const gezieneePlants = new Set();
  const rows = uploads.map(u => {
    const dt = u.created_at ? new Date(u.created_at).toLocaleString('nl-NL') : '—';
    const isLaatsteVoorPlant = !gezieneePlants.has(u.plant);
    gezieneePlants.add(u.plant);
    const magVerwijderen = accessLevel === 'admin' && isLaatsteVoorPlant;
    return `<tr>
      <td>${esc(u.label || '—')}</td>
      ...
      <td>${magVerwijderen ? `<button type="button" class="btn btn-del" style="font-size:11px;padding:3px 8px" onclick="window.__verwijderFinanceUpload('${esc(u.id)}','${esc(u.label || '')}')" title="Alleen de laatste versie van deze plant is te verwijderen">Verwijderen</button>` : ''}</td>
    </tr>`;
  }).join('');
  ...
  el.innerHTML = `<div class="tw keep-table"><table class="fin-tbl"><thead><tr><th>Versie</th><th>Plant</th><th>Jaar</th><th>Maand</th><th style="text-align:right">Regels</th><th>Door</th><th>Wanneer</th><th></th></tr></thead><tbody>${rows}</tbody></table></div>`;
}

window.__verwijderFinanceUpload = async function (uploadId, label) {
  if (!confirm(`Upload "${label}" verwijderen?\n\nDit kan alleen bij de meest recente versie van een plant, en is niet ongedaan te maken. De vorige versie wordt dan weer de getoonde stand.`)) return;
  try {
    const token = ctx && ctx.currentUser && ctx.currentUser.access_token;
    const r = await fetch(`/api/financeImport?action=delete-upload&uploadId=${encodeURIComponent(uploadId)}`, {
      method: 'DELETE',
    ...
    if (typeof window.showGlobalStatus === 'function') window.showGlobalStatus(`Upload "${label}" verwijderd.`, 'success');
  } catch (e) {
    if (typeof window.showGlobalStatus === 'function') window.showGlobalStatus('Verwijderen mislukt: ' + e.message, 'error');
    else alert('Verwijderen mislukt: ' + e.message);
  }
};
...
async function showFinance() {
  ...
  if (typeof window.requireFinanceAal2 === 'function') {
    ...
    // Fail-closed: als de globale 2FA-gate-functie onverhoopt ontbreekt (bv.
    // een laadvolgorde-regressie), moet de module dicht blijven i.p.v. door
    // te renderen zonder de aal2-check. De echte databescherming zit al
    // server-side in RLS, maar deze module presenteert zich expliciet als
    // 2FA-beveiligd en hoort dus nooit stilzwijgend die stap over te slaan.
    console.error('requireFinanceAal2 ontbreekt — Financiën blijft geblokkeerd (fail-closed).');
    if (pg) renderFinanceGate(pg);
    return;
  }
  renderPage();
}
...
export const financeModule = {
  ...
  unmount() { accessLevel = null; ctx = null; invalidateDashboardCache(); clearFinancePreferences(); remove(); }
};
