...
import { fmt, esc, MAAND, groupSum, budgetVsActual, pareto, rbmPlanActual, monthlySeries, isOnderhoudActual, versionHistory, actueelEntries } from './financeUtils.js';
...
// Consistente lege-staat i.p.v. een assenloze/lege canvas: eerder toonden
// finC8/finC9 al wel een "geen data"-melding bij een lege filtercombinatie,
// de overige grafieken (finC2-finC7) niet — nu hetzelfde patroon overal.
function emptyNote(id) {
  const cv = document.getElementById(id);
  const wrap = cv && cv.closest('.chart-container');
  if (wrap) wrap.innerHTML = '<div class="fin-note" style="margin:0">Geen data voor deze filter.</div>';
}
...
export function renderVersionHistory(el, currentUpload, others) {
  ...
  make('finVersionHistoryChart', {
    ...
    data: {
      ...
      datasets: [
        // tension:0 (rechte lijnen) i.p.v. een bezier-curve: elk punt is een
        // losse, discrete upload-snapshot — een gladgestreken curve suggereert
        // een continu verloop tússen uploads dat er niet is, en oogt bovendien
        // grillig zodra meerdere punten dicht bij elkaar liggen (herhaalde
        // maandwaarden). Rechte lijnen laten de échte stappen eerlijk zien.
        { label: 'Actual YTD', data: history.map(h => h.actYtd), borderColor: GREEN, backgroundColor: 'rgba(46,160,67,0.12)', tension: 0, borderWidth: 2.5, pointRadius: 3, pointHoverRadius: 6, pointHitRadius: 10, fill: true, spanGaps: true },
        { label: 'Plan YTD', data: history.map(h => h.planYtd), borderColor: BLUE, backgroundColor: 'transparent', tension: 0, borderWidth: 2, borderDash: [5, 4], pointRadius: 3, pointHoverRadius: 6, pointHitRadius: 10, spanGaps: true },
    ...
    // Geen beginAtZero: bij waarden die allemaal tussen bv. €2-3,6 mln liggen
    // duwde een as die bij €0 begint alle variatie samen in de bovenste helft
    // van de grafiek — precies de "schaalverdeling klopt niet"-klacht. Chart.js
    // schaalt nu automatisch rond de werkelijke waarden, met een kleine marge
    // zodat punten niet tegen de rand plakken.
    options: base({ scales: {
      x: { grid: { display: false }, ticks: { color: TICK, font: { size: 10 } } },
      y: { grid: { color: GRID }, ticks: { color: TICK, callback: v => '€' + compact.format(v) },
        grace: '10%' }
    } })
  });
}
...
export function renderCharts(el, entries, budget, projects) {
  ...
  const actueel = actueelEntries(entries);
  ...
  }); else emptyNote('finC9');

  // 2 — budget vs actual per categorie
  if (bva.length) make('finC2', {
    type: 'bar',
  ...
  if (soort.length) make('finC3', hbar(soort, GREEN)); else emptyNote('finC3');
  if (inst.length) make('finC4', hbar(inst, ORANGE)); else emptyNote('finC4');
  if (lev.length) make('finC5', hbar(lev, BLUE)); else emptyNote('finC5');

  // 6 — Pareto (bar bedrag + lijn cumulatief %)
  const parTop = par.slice(0, 12);
  if (parTop.length) make('finC6', {
    data: {
  ...
  }); else emptyNote('finC6');

  // 7 — RBM plan vs actual
  if (rbm.length) make('finC7', {
    type: 'bar',
