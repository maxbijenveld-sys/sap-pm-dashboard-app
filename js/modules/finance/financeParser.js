...
import { withinReconTolerance } from './financeUtils.js';
...
function toNum(v) {
  ...
  if (s === '' || s === '-') return null;
  // Accounting-notatie: SAP/Excel-exports schrijven negatieve bedragen soms
  // als "(1.234,56)" i.p.v. "-1.234,56". Zonder dit parseert parseFloat de
  // haakjes niet en valt de regel stil weg als "overgeslagen" — stille
  // dataverlies i.p.v. een correct negatief bedrag.
  let negParen = false;
...
const EN_MAANDEN = ['january','february','march','april','may','june','july','august','september','october','november','december'];

// ── Maint_Costs (budget/forecast vs actual, in k€) ─────────────────
function parseMaintCosts(raw) {
  // Snapshot-maand: cel rechts van "Hier de maand invullen". Dit label
  // ontbreekt op het R-tabblad (rijstructuur ligt daar één regel/kolom
  // anders — bekende R-verschuiving) — zonder fallback bleef snapshotMaand
  // voor Rotterdam altijd null. Fallback: R heeft wél een "YTD <maandnaam>"-
  // cel met het maandnummer in de cel ernaast (bv. "YTD July", 7).
  let maand = null;
  ...
  if (maand == null) {
    for (const row of raw) {
      for (let i = 0; i < (row || []).length; i++) {
        const m = norm(row[i]).match(/^ytd([a-z]+)$/);
        if (m) {
          const naastNum = toNum(row[i + 1]);
          if (naastNum != null && naastNum >= 1 && naastNum <= 12) { maand = naastNum; break; }
          const idx = EN_MAANDEN.indexOf(m[1]);
          if (idx >= 0) { maand = idx + 1; break; }
        }
      }
      if (maand != null) break;
    }
...
function parseFinanceWorkbook(sheetsByName, opts) {
  ...
  if (result.recon && result.recon.total != null) {
    ...
    if (!withinReconTolerance(diff, result.recon.total)) {
