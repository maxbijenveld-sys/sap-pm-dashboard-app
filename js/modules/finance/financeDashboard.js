...
import { fmt, esc, MAAND, applyFilters, groupSum, uniqueVals, computeKpis, isOnderhoudActual, isConsumableActual, rbmOpProjects, grandTotalBudget, rbmOrderOverlap, actueelEntries, actualByOrder, labelWithWeek } from './financeUtils.js';
    ...
    panel.innerHTML = `<div class="fin-kpi-config">${allIds.map(id => {
      ...
      return `<div class="fin-kpi-config-row" data-id="${id}">
        <div class="fin-kpi-config-left">
          <button type="button" class="fin-kpi-order" data-move="up" aria-label="Verplaats omhoog">▲</button>
          <button type="button" class="fin-kpi-order" data-move="down" aria-label="Verplaats omlaag">▼</button>
...
// Per upload_id gecachte versiedata (de "Bekijk versie"-selector bij de
...
// Render-generatie: elke renderDashboard()-aanroep claimt een eigen nummer.
// Bij een snelle plant-wissel (of dubbele aanroep) kan een oudere, tragere
// aanroep zijn data pas ontvangen ná een nieuwere — zonder deze guard
// overschrijft de oude aanroep dan alsnog de inmiddels correcte DOM met
// verouderde (verkeerde-plant) cijfers. Elke async vervolgstap checkt of
// zijn generatie nog actueel is vóórdat hij naar de DOM schrijft.
...
async function fetchEntries(ctx, uploadId, h) {
  const page = 1000;
  // kostenplaats/kostenrekening/categorie/functional_loc horen in de select:
  // zonder die kolommen vielen de rekening-labels en de bevroren categorie-
  // classificatie stil terug op naam-heuristiek (bug, gevonden bij het
  // bouwen van de kostenplaats-tabel).
  const cols = 'periode,order_nr,installatie,kostensoort,kostenrekening,kostenplaats,functional_loc,categorie,order_type,leverancier,factuur_nr,bron,bedrag_eur';
  const urlFor = offset => `${ctx.sbUrl}/rest/v1/finance_entries?upload_id=eq.${uploadId}&select=${cols}&order=id.asc&limit=${page}&offset=${offset}`;
  const getPage = async (offset, extraHeaders) => {
    const r = await fetch(urlFor(offset), { headers: { ...h, ...extraHeaders } });
    if (!r.ok) throw new Error(`kostenregels ophalen mislukte (HTTP ${r.status})`);
    return r;
  };

  // Eerste pagina met exact-count (PostgREST Content-Range-header): zo is
  // het totaal aantal pagina's vooraf bekend en kan de rest PARALLEL
  // opgehaald worden i.p.v. ronde-voor-ronde sequentieel — scheelt bij
  // grote (multi-jaar) uploads meerdere netwerk-rondetrips.
  const first = await getPage(0, { Prefer: 'count=exact' });
  const firstBatch = await first.json();
  if (firstBatch.length < page) return firstBatch; // alles paste in 1 pagina

  const range = first.headers.get('content-range') || ''; // bv. "0-999/12345"
  const total = range.includes('/') ? Number(range.split('/')[1]) : NaN;
  if (isFinite(total)) {
    const offsets = [];
    for (let o = page; o < total; o += page) offsets.push(o);
    const restBatches = await Promise.all(offsets.map(o => getPage(o).then(r => r.json())));
    return firstBatch.concat(...restBatches);
  }

  // Fallback als Content-Range onverhoopt niet bruikbaar is (bv. Prefer-
  // header genegeerd door een proxy): terugvallen op sequentieel doorgaan
  // i.p.v. gokken naar een totaal.
  let entries = firstBatch;
  let offset = page;
  while (true) {
    const batch = await getPage(offset).then(r => r.json());
...
// upload, voor de "Bekijk versie"-selector. Los van loadAll omdat dit een
// andere upload_id betreft dan de primair getoonde (nieuwste) versie.
async function loadVersionData(ctx, upload) {
  if (versionDataCache[upload.id]) return versionDataCache[upload.id];
  const token = ctx && ctx.currentUser && ctx.currentUser.access_token;
  const h = { apikey: ctx.sbKey, Authorization: 'Bearer ' + token };
  const entries = await fetchEntries(ctx, upload.id, h);
  const raw = upload.raw || {};
  const label = labelWithWeek(upload);
  const snapshotMaand = raw.snapshotMaand != null ? raw.snapshotMaand : upload.periode;
  const data = { entries, budget: raw.budget || [], projects: raw.projects || [], label, snapshotMaand };
  versionDataCache[upload.id] = data;
  return data;
}
...
// Alleen bij uitloggen aanroepen (niet na elke upload, dan zou een admin
// zijn eigen filter-/KPI-/plant-voorkeuren telkens kwijtraken). Voorkomt dat
// op een gedeeld apparaat de volgende gebruiker de filter-/KPI-/plant-
// keuzes van zijn voorganger ziet — geen bedragen, maar wel metadata
// (bv. welke installatie/leverancier gefilterd stond).
export function clearFinancePreferences() {
  try {
    if (typeof localStorage === 'undefined') return;
    localStorage.removeItem(FIN_FILTERS_KEY);
...
export async function renderDashboard(container, ctx) {
  ...
  try { data = await loadAll(ctx); }
    ...
    if (myGen !== renderGeneration) return; // een nieuwere renderDashboard-aanroep loopt al
    container.innerHTML = `<div class="fin-note fin-err">Kon financiële data niet laden: ${esc(e.message)}</div>`; return;
  }
  if (myGen !== renderGeneration) return; // overtaken — niet meer de actuele render
  ...
  container.innerHTML = `
    <div class="fin-card">
      <div class="fin-toolbar">
        <div class="fin-sub" id="finVersionSub"></div>
      ...
      <div class="fin-tabs" role="tablist">
        <button class="fin-tab active" id="finTabBtn-overzicht" data-tab="overzicht" role="tab" aria-selected="true" aria-controls="finTab-overzicht">&#128200; Overzicht</button>
        <button class="fin-tab" id="finTabBtn-analyse" data-tab="analyse" role="tab" aria-selected="false" aria-controls="finTab-analyse">&#128269; Analyse</button>
        <button class="fin-tab" id="finTabBtn-bronnen" data-tab="bronnen" role="tab" aria-selected="false" aria-controls="finTab-bronnen">&#128230; Bronnen &amp; uitleg</button>
      </div>
    </div>

    <div class="fin-tabpane" id="finTab-overzicht" role="tabpanel" aria-labelledby="finTabBtn-overzicht">
    ...
    <div class="fin-tabpane" id="finTab-analyse" role="tabpanel" aria-labelledby="finTabBtn-analyse" style="display:none">
      <div class="fin-card">
    ...
    <div class="fin-tabpane" id="finTab-bronnen" role="tabpanel" aria-labelledby="finTabBtn-bronnen" style="display:none">
      <div class="fin-card">
  ...
  // Overzicht + Bronnen: standaard op de nieuwste (live) versie. Via de
  // "Bekijk versie"-selector kan de gebruiker een oudere upload kiezen —
  // dan wisselt state.entries/budget/projects helemaal om (zie
  // selectViewedUpload), niet alleen een los vergelijkingscijfer, zodat
  // ELKE kaart/tabel/grafiek op het dashboard die versie toont.
  const liveVersion = { entries: data.entries, budget: state.budget, projects: state.projects, snapshotMaand: state.snapshotMaand, label: data.upload.label, entryCount: data.entries.length, upload: data.upload };
  state.others = data.others || [];
  state.viewedUploadId = '';
  state.viewedLabel = liveVersion.label;
  state.upload = liveVersion.upload;
  state.analyseRendered = false;
  renderKpiConfig(container.querySelector('#finKpiConfig'), () => renderKpis(container, state));
  renderVersionSelector(container.querySelector('#finCompareHost'), state, id => selectViewedUpload(container, ctx, state, id, myGen, liveVersion, plant));
  refreshDataDependentViews(container, state, plant, liveVersion.label, liveVersion.entryCount, true);
  ...
  container.querySelectorAll('.fin-tab').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (btn.dataset.tab === 'analyse' && !state.analyseRendered) { renderAnalyseTab(container, state); state.analyseRendered = true; }
    });
  });
  ...
  // Export respecteert de huidige filters én de bekeken versie (niet altijd
  // de nieuwste — state.viewedLabel volgt de "Bekijk versie"-selector).
  ...
  if (expXlsx) expXlsx.addEventListener('click', () => exportExcel(
    ...
    `financien_${plant}_${(state.viewedLabel || 'export').replace(/[^\w]+/g, '_')}.xlsx`
  ));
...
function kpiCard(label, val, sub, tip, accent, cat, id, hero) {
  ...
  return `<div class="${cls}"${accent ? ` style="--kpi:${accent}"` : ''}${attrs}>${info}${tag}<div class="fin-kpi-l">${esc(label)}</div><div class="fin-kpi-v">${val}</div>${sub ? `<div class="fin-kpi-s">${sub}</div>` : ''}</div>`;
}
...
function renderSelectedKpis(k, ids) {
  return ids.map(id => {
    const def = KPI_DEFS[id];
    if (!def) return '';
    ...
    return kpiCard(def.label, value, sub, def.tip, def.accent, def.cat, id, id === 'actual');
  }).join('');
}

// ── Versie bekijken: selector + laden ────────────────────────────────
// Voorheen "vergelijk met": een gekozen oudere versie toonde alleen een los
// deltacijfer onder elke KPI-kaart, terwijl de rest van het dashboard altijd
// de nieuwste versie bleef tonen. Dat gaf een verwarrend hybride beeld. Nu
// is de selector een echte periode-wissel: kies je een andere versie, dan
// draait het HELE dashboard (KPI's, budgetstatus, grafieken, inzichten,
// bronnen, Analyse-tab) om op die versie — zie selectViewedUpload en
// refreshDataDependentViews. "Verloop over tijd" (renderVersionHistory) is
// de uitzondering: die toont bewust altijd de trend over ALLE versies, dus
// verandert niet mee met de keuze hier.
function renderVersionSelector(host, state, onChange) {
  if (!host) return;
  const others = state.others || [];
  if (!others.length) { host.innerHTML = ''; return; }
  const cur = state.viewedUploadId || '';
  host.innerHTML = `<label class="fin-compare-lbl">Bekijk versie
    <select class="fin-inp" id="finVersionSel">
      <option value=""${cur === '' ? ' selected' : ''}>Huidige versie</option>
      ${others.map(u => {
        const lbl = labelWithWeek(u);
        return `<option value="${u.id}"${String(u.id) === String(cur) ? ' selected' : ''}>${esc(lbl)}</option>`;
      }).join('')}
    </select>
  </label>`;
  const sel = host.querySelector('#finVersionSel');
  sel.addEventListener('change', () => onChange(sel.value || null));
}
async function selectViewedUpload(container, ctx, state, uploadId, myGen, liveVersion, plant) {
  const host = container.querySelector('#finCompareHost');
  const sel = host && host.querySelector('select');
  const backToLive = () => {
    state.entries = liveVersion.entries; state.budget = liveVersion.budget; state.projects = liveVersion.projects;
    state.snapshotMaand = liveVersion.snapshotMaand; state.viewedUploadId = ''; state.viewedLabel = liveVersion.label;
    state.upload = liveVersion.upload;
    if (myGen === renderGeneration) refreshDataDependentViews(container, state, plant, liveVersion.label, liveVersion.entryCount, true);
  };
  if (!uploadId) { backToLive(); return; }
  const upload = (state.others || []).find(u => String(u.id) === String(uploadId));
  if (!upload) { backToLive(); return; }
  if (sel) sel.disabled = true;
  try {
    const v = await loadVersionData(ctx, upload);
    state.entries = v.entries; state.budget = v.budget; state.projects = v.projects;
    state.snapshotMaand = v.snapshotMaand; state.viewedUploadId = upload.id; state.viewedLabel = v.label;
    state.upload = upload;
    if (myGen === renderGeneration) refreshDataDependentViews(container, state, plant, v.label, v.entries.length, false);
  } catch (e) {
    console.error('Versie laden faalde:', e);
    if (sel) sel.value = '';
    backToLive();
  } finally {
...
// Alles wat van state.entries/budget/projects afhangt in één keer
// herrenderen — gebruikt bij de eerste render én bij elke versie-wissel,
// zodat er maar één plek is die weet welke onderdelen van het dashboard
// meebewegen met de gekozen versie.
function refreshDataDependentViews(container, state, plant, viewLabel, entryCount, isLive) {
  state.kFull = computeKpis(state.entries, state.budget, state.projects);
  renderVersionSub(container, plant, viewLabel, entryCount, isLive);
  ...
  renderAnalysis(container.querySelector('#finInsights'), state.entries, state.budget, state.projects, state.snapshotMaand);
  renderSources(container.querySelector('#finSources'), { entries: state.entries, budget: state.budget, projects: state.projects, orderMap: state.orderMap, plant });
  buildFilters(container.querySelector('#finFilters'), state, () => renderAnalyseTab(container, state));
  if (state.analyseRendered) renderAnalyseTab(container, state);
}
function renderVersionSub(container, plant, label, entryCount, isLive) {
  const el = container.querySelector('#finVersionSub');
  if (!el) return;
  el.innerHTML = `<strong>${esc(PLANT_NAMES[plant])}</strong> · versie: <strong>${esc(label || '—')}</strong> · ${entryCount} regels${isLive ? '' : ' <span class="fin-version-badge">historische versie — niet de actuele stand</span>'}`;
}
...
function renderBudgetStatus(el, k, budget) {
  ...
  // Ruwe percentages voor de legenda-tekst (mag negatief/>100 tonen — dat is
  // dan feitelijk correcte info, bv. een netto-creditperiode of overschrijding).
  const bestP = pctOf(besteed, b), oblP = pctOf(obligo, b);
  // Geclampte breedtes voor de balksegmenten zelf: een negatieve of >100%
  // waarde geeft een ongeldige/kapotte CSS-width en segmenten die niet meer
  // optellen tot 100%. Besteed en obligo mogen samen nooit boven 100% van de
  // balk komen, ongeacht wat de werkelijke (mogelijk hogere) bedragen zijn.
  const bestPBar = Math.max(0, Math.min(100, bestP));
  const oblPBar = Math.max(0, Math.min(100 - bestPBar, oblP));
  const vrijP = Math.max(0, 100 - bestPBar - oblPBar);
  const fcPos = fc != null ? Math.max(0, Math.min(100, pctOf(fc, b))) : null;
  ...
  el.innerHTML = `
    ...
    <div class="fin-bar" title="Verdeling van het jaarbudget">
      <div class="fin-bar-track">
        <div class="fin-bar-seg" style="width:${bestPBar}%;background:#2ea043"></div>
        <div class="fin-bar-seg" style="width:${oblPBar}%;background:#d29922"></div>
...
function renderKpis(container, state) {
  ...
  host.innerHTML = `<div class="fin-kg">${renderSelectedKpis(state.kFull, selectedKpis)}</div>`;
  ...
  host.querySelectorAll('.fin-kpi[data-kpi]').forEach(card => {
    ...
    card.addEventListener('click', e => {
      ...
      if (id === 'rbmBlokken') renderRbmTable(detail.querySelector('#finKdRbmTable'), state.projects, state.entries);
    });
  });
}

// Klik-om-uit-te-klappen voor elke tabel die kdTable() met een `details`-
// array opbouwt (.reden-row/.reden-detail-row/.tog) — één plek voor dit
// gedrag, i.p.v. het per tabel-container opnieuw te bedraden. Toepasbaar op
// elk element dat zulke rijen bevat (KPI-detailpaneel, Analyse-tabellen, ...).
function wireExpandableRows(container) {
  container.querySelectorAll('.reden-row').forEach(row => {
    const toggle = () => {
...
// Groepstabel (Analyse-tab: kostensoort/installatie/kostenplaats) mét
// uitklapbare rijen — klik op een rij toont de top-10 onderliggende
// boekingen (grootste bedrag eerst), zelfde patroon als de KPI-
// detailpanelen. `groupField` bepaalt op welk entry-veld wordt
// teruggefilterd om de regels ván die groep op te halen.
function tblExpandable(el, groupRows, sourceEntries, groupField, labelHead, labelFn, detailHead, detailRowFn) {
  if (!el) return;
  if (!groupRows.length) { el.innerHTML = '<div class="fin-note">Geen data voor deze filter.</div>'; return; }
  const rows = groupRows.map(r => [labelFn ? labelFn(r.key) : esc(r.key), fmt(r.bedrag)]);
  const details = groupRows.map(r => kdEntryDetail(
    sourceEntries.filter(e => (e[groupField] || '—') === r.key),
    labelFn ? labelFn(r.key) : r.key, detailHead, detailRowFn
  ));
  el.innerHTML = kdTable([labelHead, 'Bedrag'], rows, details);
...
function kpiDetailHtml(id, state) {
  ...
  if (id === 'actual') {
    ...
    blocks =
      ...
      kdBlock('Berekening', `<strong>${k.orders}</strong> unieke ordernummers over alle bronnen (CCTR, KOB1 en KOB2). Daarvan zijn <strong>${ondOrders.size}</strong> afkomstig uit onderhoud-actuals — ${gekoppeld} daarvan zijn ook terug te vinden in de weekplanning (koppeling via het ordernummer).`);
...
let _rbmBlok = 'alle';
function renderRbmTable(el, projects, entries) {
  if (!el) return;
  ...
  // RBM-tab wordt periodiek handmatig bijgewerkt en loopt soms achter op de
  // echte CCTR/KOB1-boekingen op dezelfde order (bv. DA4814: order wel
  // ingevuld, maar RBM-realisatie nog op 0 terwijl er al kosten geboekt
  // zijn) — vul aan met wat SAP al echt heeft geboekt, i.p.v. dat de tabel
  // "geen kosten" laat zien terwijl dat niet klopt.
  const byOrder = actualByOrder(entries);
  ...
  // Blokkolom/-filter alleen tonen als er meer dan één blok in de data zit
  // — bij oudere uploads (vóór parser 2.2.0, alleen OP) is een kolom met
  // dezelfde badge op elke rij nutteloze ruis.
  const toonBlok = blokken.length > 1;
  const filterHtml = blokken.length > 1
    ...
    : '';
  let anyBijgewerkt = false;
  const rows = sorted.map(p => {
    const plan = Number(p.plan_eur) || 0;
    const rbmAct = Number(p.actual_eur) || 0;
    const boekt = p.order_nr ? (byOrder[p.order_nr] || 0) : 0;
    const act = Math.max(rbmAct, boekt);
    const bijgewerkt = boekt > rbmAct;
    if (bijgewerkt) anyBijgewerkt = true;
    const pct = plan > 0 ? Math.min(100, Math.round(act / plan * 100)) : (act > 0 ? 100 : 0);
    const over = plan > 0 && act > plan;
    // Variantie uit de RBM-tab is leidend zolang die klopt met de RBM-eigen
    // realisatie; alleen als "bijgewerkt" met CCTR/KOB1 herberekenen, anders
    // klopt de variantie niet meer met het getoonde (hogere) actual-bedrag.
    const varc = bijgewerkt ? (plan - act) : (p.variance_eur != null ? p.variance_eur : plan - act);
    return `<tr>
      <td>${esc(p.naam)}</td>
      ${toonBlok ? `<td>${blokBadge(p)}</td>` : ''}
      <td style="font-family:monospace;font-size:12px">${esc(p.order_nr || '—')}</td>
      <td style="text-align:right">${fmt(plan)}</td>
      <td style="text-align:right"${bijgewerkt ? ` title="RBM-tab toont nog ${fmt(rbmAct)}; aangevuld met ${fmt(boekt)} al geboekt op order ${esc(p.order_nr)} in CCTR/KOB1"` : ''}>${fmt(act)}${bijgewerkt ? ' <span style="opacity:.6">*</span>' : ''}</td>
  ...
  el.innerHTML = `${filterHtml}<div class="tw keep-table"><table class="fin-tbl fin-tbl-rbm"><thead><tr>
    ${th('naam', 'Project')}${toonBlok ? th('blok', 'Blok') : ''}${th('order_nr', 'Order')}${th('plan_eur', 'Plan', true)}${th('actual_eur', 'Realisatie', true)}<th>Voortgang</th>${th('variance_eur', 'Variantie', true)}
  </tr></thead><tbody>${rows}</tbody></table></div>${anyBijgewerkt ? '<div class="fin-note">* Realisatie aangevuld met kosten die al op deze order geboekt staan in CCTR/KOB1, maar nog niet in het RBM-tabblad zelf — beweeg over het bedrag voor details.</div>' : ''}`;
  el.querySelectorAll('th[data-sort]').forEach(h => h.addEventListener('click', () => {
    const k = h.dataset.sort;
    _rbmSort = { key: k, dir: _rbmSort.key === k ? -_rbmSort.dir : 1 };
    renderRbmTable(el, projects, entries);
  }));
...
function renderAnalyseTab(container, state) {
  ...
  const orderCell = x => x.order_nr ? `<span style="font-family:monospace;font-size:12px">${esc(x.order_nr)}</span>` : '—';

  const actueel = actueelEntries(filtered);
  // Elke tabel hier is nu klikbaar: een rij openen toont de top-10 grootste
  // onderliggende boekingen binnen die groep — het eigen gegroepeerde veld
  // wordt in de detailkolommen weggelaten (staat al als rijkop, dubbelop anders).
  tblExpandable(
    container.querySelector('#finTblSoort'), groupSum(actueel, 'kostensoort').slice(0, 15),
    actueel, 'kostensoort', 'Kostensoort', key => kostensoortLabel(actueel, key),
    ['Order', 'Installatie', 'Bedrag'], x => [orderCell(x), esc(x.installatie || x.functional_loc || '—'), fmt(x.bedrag_eur)]
  );
  tblExpandable(
    container.querySelector('#finTblInst'), groupSum(actueel.filter(e => e.installatie), 'installatie').slice(0, 10),
    actueel, 'installatie', 'Installatie', null,
    ['Order', 'Kostensoort', 'Bedrag'], x => [orderCell(x), esc(x.kostensoort || '—'), fmt(x.bedrag_eur)]
  );
  const ccEl = container.querySelector('#finTblCC');
  if (ccEl) tblExpandable(
    ccEl, groupSum(actueel.filter(e => e.kostenplaats), 'kostenplaats').slice(0, 10),
    actueel, 'kostenplaats', 'Kostenplaats', key => ccLabel(state, key),
    ['Order', 'Kostensoort', 'Installatie', 'Bedrag'], x => [orderCell(x), esc(x.kostensoort || '—'), esc(x.installatie || x.functional_loc || '—'), fmt(x.bedrag_eur)]
  );
  ...
  renderRbmTable(container.querySelector('#finRbmTable'), state.projects, state.entries);
}
