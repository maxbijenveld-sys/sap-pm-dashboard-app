...
import { fmt, esc, MAAND, applyFilters, groupSum, uniqueVals, computeKpis, isOnderhoudActual, isConsumableActual, rbmOpProjects, grandTotalBudget, rbmOrderOverlap, actueelEntries } from './financeUtils.js';
...
export async function renderDashboard(container, ctx) {
  ...
  renderCompareSelector(container.querySelector('#finCompareHost'), state, id => selectCompareUpload(container, ctx, state, id, myGen));
  if (state.pendingCompareId) selectCompareUpload(container, ctx, state, state.pendingCompareId, myGen);
...
async function selectCompareUpload(container, ctx, state, uploadId, myGen) {
  ...
  if (!uploadId) {
    ...
    if (myGen === renderGeneration) renderKpis(container, state);
    return;
  }
  ...
  try {
    ...
    // Guard: als er inmiddels een nieuwere renderDashboard()-aanroep is
    // gestart (bv. plant-wissel tijdens het laden), hoort deze verouderde
    // vergelijkingsdata niet meer in de (inmiddels vervangen) DOM te landen.
    if (myGen === renderGeneration) renderKpis(container, state);
  }
}
