...
import { fmt, esc, MAAND, isOnderhoudActual, weekIndex } from './financeUtils.js';
...
export async function loadMaintenanceOrders(ctx) {
  ...
  if (!token) return {};
  try {
    const r = await fetch(`${ctx.sbUrl}/rest/v1/weeks?select=week,year,rows&order=year.asc,week.asc`, { headers: { apikey: ctx.sbKey, Authorization: 'Bearer ' + token } });
    if (!r.ok) return {};
    ...
    for (const w of (weeks || [])) {
      for (const row of (w.rows || [])) {
        ...
        if (!wo || wo === '—') continue;
        const prev = map[wo];
        // Jaar-bewust vergelijken (weekIndex) i.p.v. kaal weeknummer — anders
        // zou bv. W2 2027 als "ouder" dan W51 2026 worden behandeld en de
        // laatst bekende status/omschrijving verkeerd om overschrijven.
        if (!prev || weekIndex(w.year, w.week) >= weekIndex(prev._year, prev._week)) {
          map[wo] = { omschrijving: row.omschrijving, status: row.status, type: row.type, _week: w.week, _year: w.year, voorkomens: (prev ? prev.voorkomens : 0) + 1 };
