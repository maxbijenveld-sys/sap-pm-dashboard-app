...
import { fmt, esc, isOnderhoudActual, isConsumableActual, grandTotalBudget, withinReconTolerance } from './financeUtils.js';
...
export function renderSources(el, { entries, budget, projects, orderMap, plant }) {
  if (!el) return;
  ...
  // Tabbladprefix volgt de actieve plant (IJ_/R_) — anders toont deze kaart
  // voor Rotterdam-data nog altijd de IJmuiden-tabbladnamen.
  const p = plant || 'IJ';

  // Reconciliatie: detail (CCTR+KOB1) versus de Maint_Costs-kopregel.
  const recoDiff = (s.actYtd != null) ? s.actueel - s.actYtd : null;
  const recoOk = recoDiff != null && withinReconTolerance(recoDiff, s.actYtd);
  const vrijOp = (s.budgetOp != null) ? s.budgetOp - s.vastgelegd : null;

  const cards = [
    srcCard(`${p}_Maint_Costs`, 'Budget & forecast', 'OP/FC vs. werkelijk en plan', s.budgetOp, 'OP', `${s.budgetLines} regels`, '#388bfd', 'Maint_Costs'),
    srcCard(`${p}_CCTR`, 'Kostenplaats-kosten', 'werkelijke kosten op kostenplaatsen', s.cctr.total, '', `${s.cctr.count} regels`, '#2ea043', 'CCTR'),
    srcCard(`${p}_KOB1`, 'Werkorder-kosten', 'werkelijke kosten op onderhoudsorders', s.kob1.total, '', `${s.kob1.count} regels`, '#2ea043', 'KOB1'),
    srcCard(`${p}_KOB2`, 'Open obligo', 'vastgelegde, nog niet gefactureerde kosten', s.kob2.total, '', `${s.kob2.count} regels`, '#d29922', 'KOB2'),
    srcCard(`${p}_Personeel`, 'Personeelskosten', 'loonkosten: vast, overwerk, inhuur', s.pers.total, '', `${s.pers.count} regels`, '#8b5cf6', 'Personeel'),
    srcCard(`${p}_RBM`, 'RBM-projecten', 'projectmatig onderhoud: plan vs. actual', s.projActual, 'actual', `${s.projCount} projecten`, '#06b6d4', 'RBM')
  ].join('');
