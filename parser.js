...
// Alle 7 dagen (index = JS getUTCDay(): 0=zondag) — nodig om een dagmarkering
// robuust te labelen op basis van de échte kalenderdag i.p.v. blind op de
// n-de positie in de week te gokken (zie dagLabelFromSerial hieronder).
const ALLE_DAGNAMEN=['Zo','Ma','Di','Wo','Do','Vr','Za'];
// Excel-datumserienummer (dagen sinds 1899-12-30, met de bekende Excel-
// leap-year-eigenaardigheid) -> dagnaam. Gebruikt zodat een onverwacht 6e/7e
// dagblok (bv. een zaterdagploeg) de juiste naam krijgt i.p.v. stil als de
// laatst bekende naam (Vr) meegeteld te worden.
function dagLabelFromSerial(serial){
  const d=new Date(Date.UTC(1899,11,30)+Number(serial)*86400000);
  return ALLE_DAGNAMEN[d.getUTCDay()];
}
...
function parseExcelSheet(raw){
  // Fallback op het actuele kalenderjaar i.p.v. een hardgecodeerd jaartal —
  // een sheet zonder herkenbare "Jaar"-cel zou anders elk jaar stil hetzelfde
  // (inmiddels verkeerde) jaartal blijven krijgen, met verkeerde aging/
  // risicoscore/backlog-sortering tot gevolg (die allemaal op _year steunen).
  let weekNum=null, yearNum=new Date().getFullYear();
  for(let i=0;i<Math.min(10,raw.length);i++){
    const r=raw[i]||[];
    for(let j=0;j<r.length;j++){
      const v=(r[j]||'').toString().trim().toLowerCase();
      if(v==='week'&&r[j+1]) weekNum=parseInt(r[j+1]);
      if(v==='jaar'&&r[j+1]) yearNum=parseInt(r[j+1])||yearNum;
    }
  }
  ...
  // gepland als (indien aanwezig) ongepland. Vanaf W30 heet de gepland-kant
  // "Reden (niet uitgevoerd)" i.p.v. kaal "Reden" — een exacte match miste
  // die kolom dus volledig (de reden bleef stil leeg voor elke "nee"-regel).
  // startsWith i.p.v. exacte match vangt beide varianten én toekomstige
  // tekstuitbreidingen, zolang de kolom met "reden" begint.
  let redenCols=[];
  for(const r of raw){
    for(let ci=0;ci<(r||[]).length;ci++){
      if((r[ci]||'').toString().trim().toLowerCase().startsWith('reden')) redenCols.push(ci);
    }
  ...
  const addRow=(omschr,order,w1,w2,w3,stat,reden,opmerking,type)=>{
    ...
    // Sommige exports herhalen de kolomkop per dag met een afwijkende
    // omschrijving (bv. "Werkzaamheden . Omschrijving volgens SAP" i.p.v.
    // de kale "Werkzaamheden" die al in skipVals staat) — die tekst matcht
    // skipVals niet en lekte door als een nep-werkorder, één per dag. De
    // order-kolom van zo'n headerrij is altijd letterlijk "Order" (nooit een
    // echt ordernummer), dus dat is een robuustere check dan elke mogelijke
    // tekstvariant los op te sommen.
    if((order||'').toString().trim().toLowerCase()==='order') return;
  ...
  for(let i=0;i<raw.length;i++){
    ...
    // Dagmarkering: Excel-datumserienummer in kolom A. Dagnaam komt uit het
    // serienummer zelf (echte kalenderdag), niet uit de positie — een extra
    // dagblok (bv. een zaterdagploeg) krijgt zo zijn eigen, juiste label
    // i.p.v. stilzwijgend als "Vr" (de laatste in de vaste 5-daagse lijst)
    // meegeteld te worden.
    if(/^\d{5}$/.test(c0)){
      currentDag=dagLabelFromSerial(c0);
      continue;
    }
    // "Werkzaamheden (vulwerk)": een reservelijst ná het laatste dagblok, niet
    // aan een specifieke dag gekoppeld. Zonder deze check erfden die regels
    // stilzwijgend de laatst bekende dag (meestal Vr) — ze telden dan mee als
    // vrijdagwerk in de per-dag-weergaven, terwijl het invulwerk is voor als
    // er ruimte is, niet dagspecifiek gepland.
    if(c0.toLowerCase()==='werkzaamheden (vulwerk)'){
      currentDag='Vulwerk';
      continue;
    }
...
// Eén doorlopend, jaar-bewust weeknummer — voor "hoe lang geleden"/aging-
// berekeningen (backlog "weken open", risicoscore) die een jaarwisseling
// moeten kunnen overbruggen zonder negatief of absurd klein te worden.
// Kaal weeknummer alleen ('w19') is niet uniek over jaren heen: week 19 van
// 2027 is een andere week dan week 19 van 2026, maar `last._week-first._week`
// zag ze als "hetzelfde punt" — bij een order dat van wk51 (2026) naar wk2
// (2027) doorloopt gaf dat 2-51+1=-48. 53 als opslagfactor (bovengrens
// ISO-weken/jaar) hoeft niet exact te zijn, alleen groot genoeg en overal
// consistent gebruikt. Ontbrekend jaar telt als 0 (bewust, geen throw) —
// zo blijven oudere aanroepen die nog geen jaar meegeven onderling correct
// vergelijkbaar (allebei 0), ze worden alleen fout zodra ze een échte
// jaargrens oversteken, wat zonder jaar toch al niet detecteerbaar was.
function weekIndex(year,week){ return (Number(year)||0)*53+(Number(week)||0); }
...
if (typeof module !== 'undefined' && module.exports) {
  module.exports = { parseStatus, DAGNAMEN, parseExcelSheet, isoWeekOf, isoWeekBefore, isoWeekJustFinished, weekIndex };
}
